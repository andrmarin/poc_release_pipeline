...
#
# Usage: verify.sh <path-to-zip> <expected-environment>
set -euo pipefail

fail() { echo "VERIFY FAILED: $*" >&2; exit 1; }

[[ $# -eq 2 ]] || fail "usage: verify.sh <path-to-zip> <expected-environment>"
zip_path="$1"
expected_env="$2"
...
echo "ok: build.info valid (environment=$environment version=$version)"
...
echo "VERIFY PASSED: $zip_name ($expected_env)"
